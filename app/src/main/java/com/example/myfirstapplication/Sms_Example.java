...
public class Sms_Example extends AppCompatActivity implements View.OnClickListener {
   ...
   Button btnsend;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        edtmno = (EditText) findViewById(R.id.editText_mno);
        edtmsg = (EditText) findViewById(R.id.editText_msg);
    ...
    public void onClick(View view) {
        if(view == btnsend)
        {
            String mno = edtmno.getText().toString().trim();
            String msg = edtmsg.getText().toString().trim();

            Intent i = new Intent(getApplicationContext(),Sms_Example.class);
            PendingIntent pi = PendingIntent.getActivity(getApplicationContext(),0,i,0);

            SmsManager sm = SmsManager.getDefault();
            sm.sendTextMessage(mno,null,msg,pi,null);
